...
def make_folds():
    ...
    np.save(f'data/{NUM_FOLDS}_tiff_folds', np.array([np.array(f) for f in folds]))
...
def plot_folds():
    ...
    folds = np.load(f'data/{NUM_FOLDS}_tiff_folds.npy')
    assert np.concatenate([np.array(f) for f in folds], axis=0).shape[0] == df.shape[0]
    assert np.all(np.equal(folds[0][:3], [3152, 31495, 18964]))
    assert np.all(np.equal(folds[-1][-3:], [22790, 36269, 7618]))
...
def make_splits():
    ...
    for i, fold in enumerate(folds):
        os.makedirs(f'data/fold{i}_tiff', exist_ok=True)
        train = df.iloc[list(set(range(df.shape[0])) - set(fold))].copy()
        train['path'] = train['path'].apply(lambda s: os.path.join(prefix, 'train-tif-v2', s) + '.tif')
        train.to_csv(f'data/fold{i}_tiff/train.csv', index=False)
        val = df.iloc[fold].copy()
        val['path'] = val['path'].apply(lambda s: os.path.join(prefix, 'train-tif-v2', s) + '.tif')
        val.to_csv(f'data/fold{i}_tiff/val.csv', index=False)
